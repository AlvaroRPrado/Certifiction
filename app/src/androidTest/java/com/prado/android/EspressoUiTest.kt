...
//        )
    ...
    @Test
    fun testIncrement(){
        //lounch desired activity - OLD WAI NO CASO DE VC ENCONTRAR ISSO POR AI
    ...
    fun testIncrementNewWithRuleEvenBetterSinceItClosesScenarioAutomatically(){
       ...
       scenario.onActivity { activity ->
           println("se quiser chamar algum metdo da activity, faz aqui")//do some with the activity
            //exemplo
            //activity.onBackPressed()
        }
        Espresso.onView(withId(R.id.codelab_accessibility)).perform(ViewActions.click())
    ...
    fun useAppContext() {
        ...
        assertEquals("com.prado.android", appContext.packageName)
    }
}